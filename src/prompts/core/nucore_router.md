...
2. If you **can** determine the intent:
  21. **Find relevant devices** - Search the device database and score each device's relevance to the query:
  ...
  22. **Call nucore_router_tool** with:
   - The intent
   - **All** relevant devices regardless of their score 

  23. **CRITICAL Multi-Intent Handling**: If a query has multiple distinct intents, call the tool MULTIPLE times - once per intent.
    **Examples:**
