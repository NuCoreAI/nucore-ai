...
class EditorMinMaxRange:
    ...
    def get_names(self, is_json):
        """
        Returns a dictionary of names for the range.
        """
        if is_json:
            return self.get_json_names()
...
class Editor:
    ...
    def get_json_descriptions(self):
        ...
        if self.is_reference:
            out=(f"\"editor\":{{\"id\":\"{REFERENCE_DELIMITER} id={self.id}\"}}")
            return out 
        if len(self.ranges) == 0: 
            return out
        out += f"\"editor\":{{\"id\":\"{self.id}\",\"ranges\":["
    ...
    def write_descriptions(self, writer):
        ...
        if self.is_reference:
            with writer.block():
                writer.write(f"editor id={REFERENCE_DELIMITER} id={self.id}")
            return
        if len(self.ranges) == 0: 
            return
        with writer.block():
            writer.write(f"editor id={self.id}:")
    ...
    def write_prompt_section(self, writer, USE_JSON_OUTPUT):
        ...
        with writer.block():
            ...
            if USE_JSON_OUTPUT:
                writer.write(self.get_json_prompt())
                return
            for r in self.ranges:
                r.write_description(writer)  
