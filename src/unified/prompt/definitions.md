...
keeps rebooting"), call `get_diagnostics_prompt` first for how to investigate (INSTEON link
reasoning, known fixes, which tool to use), then call whichever of `get_full_system_config`/
`get_device_family`/`get_dev_links_table`/`get_iox_links_table`/`compare_device_links`/
`get_all_plm_links`/`quick_plm_sanity_check` fit — these are ordinary tools, always available, no
...
flow above: call `get_diagnostics_prompt` for the service catalog and the exact
`run_shell_command` invocation pattern.
